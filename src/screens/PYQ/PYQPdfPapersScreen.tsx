...
export default function PYQPdfPapersScreen({ navigation }: Props) {
  ...
  const loadPapers = async () => {
    try {
      ...
      console.error('[PYQPdfPapersScreen] load error', e);
      setError('Failed to load previous year papers.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <View style={styles.cCenter}>
        ...
        <Text style={styles.centerText}>Loading previous year papers…</Text>
      </View>
    );
  }
