...
export default function CustomMCQQuizScreen({ navigation }: Props) {
            ...
            snap => {
                const unitNodes: UnitNode[] = snap.docs.map(d => {
                    ...
                    return {
                        ...
                        name: data.name ?? '(No name)',
                        type: 'unit',
                        parentId: data.parentId, // subjectId
                        subjectId: data.subjectId ?? data.parentId,
                        order: data.order ?? 0,
                    };
                });
            ...
            snap => {
                const chapNodes: ChapterNode[] = snap.docs.map(d => {
                    ...
                    return {
                        ...
                        unitId: data.unitId ?? data.parentId,
                        questions,
                    };
                });
    ...
    const handleStartQuiz = () => {
        ...
        targetChapters.forEach(ch => {
            (ch.questions || []).forEach(qObj => {
                collected.push({
                    ...qObj,
                    id: qObj.id || `${ch.id}_${collected.length}`,
                });
            });
        });
